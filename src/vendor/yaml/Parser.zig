...
fn listBracketed(self: *Parser, gpa: Allocator) ParseError!Node.OptionalIndex {
    ...
    // devrun patch — see src/vendor/yaml/PROVENANCE.md.
    //
    // This guard exists to reject `[a, b]# c`, where the comment touches the
    // closing bracket. But `eatToken` runs `eatCommentsAndSpace` first, which
    // skips `.new_line`, so before the line check below it also rejected an
    // ordinary comment on the *following* line:
    //
    //     dotenv: [".env"]
    //     # perfectly valid YAML
    //     command: "..."
    //
    // Only a comment sharing the closing bracket's line is unseparated.
    if (self.eatToken(.comment, &.{.comment})) |pos| {
        if (self.getLine(pos) == self.getLine(node_end)) {
