...
pub const Archive = struct {
    ...
    ///
    /// `index_path` gets the `.idx` sidecar, which belongs in the same Session
    /// directory as the log it describes: its offsets are only meaningful
    /// against these bytes. Null skips it, and a sidecar that cannot be created
    /// is skipped too rather than failing the Archive — the log is the record,
    /// the Index only makes it faster to query.
    pub fn create(
        gpa: Allocator,
        path: [*:0]const u8,
